...
def chatgpt_function(prompt: str):
    ...
    functions = [
        ...
        {
            "name": "get_wiki",
            "description": f"通过wiki搜索接口获得准确的百科信息",
            "parameters": {
                "type": "object",
                "properties": {
                    "query": {
    ...
    if (message.get("function_call")):
        ...
        if (function_name == "get_wiki"):
            try:
                query = arguments['query']
                wiki_summary = get_wiki(query)
                print(f"wiki_summary success,query : {query}")
                return wiki_summary
            except Exception as e:
                print(f"An error occurred: {e}")
                return f"An error occurred: {e}"
...
def get_wiki(query:str) -> str:
    """
    根据提供的搜索查询，使用wikipedia进行检索，再使用chatgpt总结搜索结果。

    参数:
    query: 用户提供的搜索查询字符串。

    返回:
    wiki_summary:根据搜索结果,chatgpt生成的摘要。
    """
    raw_content = tools.get_wikipedia_text(query)
    return f"基于{query}在wiki百科上进行了搜索，结果为:\n{raw_content}"

if __name__ == "__main__":
    query = "在wiki查询一下米哈游的情况"
